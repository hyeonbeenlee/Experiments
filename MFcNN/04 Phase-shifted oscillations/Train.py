...
def f_yh(x):
	y = x ** 2 + torch.sin(8 * torch.pi * x + torch.pi / 10)
	return y

# TrainData
x_lo = torch.linspace(0, 1, 51).view(-1, 1)
y_lo = f_yl(x_lo)
dy_lo = f_dyl(x_lo)

x_hi = torch.linspace(0, 1, 16).view(-1, 1)
y_hi = f_yh(x_hi)
...
ax.grid()
ax.legend(loc=4)
ax.set(xlim=(0, 1), ylim=(-3, 2), xlabel='$x$', ylabel='$y$')

# Low-fidelity
NN_l = C.StackedNN(1, 4, 20, 1)
...
ax.scatter(x_lo, y_lo, marker='o', facecolors='none', edgecolors='blue', s=100, linewidth=2, label='Low-fidelity training data')
ax.scatter(x_hi, y_hi, marker='x', c='red', s=100, linewidth=2, label='High-fidelity training data')
ax.plot(test_x, prediction_yh, label='Multi-fidelity learned CNN (Method I)', color='gold', linewidth=3)
ax.grid()
ax.legend(loc=4)
ax.set(xlim=(0, 1), ylim=(-3, 2), xlabel='$x$', ylabel='$y$')

# Multi-fidelity (Method II)
Epochs = 15000
l2_lambda = 0.01

NN_l = C.StackedNN(1, 4, 20, 1)
NN_h1 = C.StackedNN(2, 1, 10, 1, activation='none')
NN_h2 = C.StackedNN(3, 2, 10, 1)

Par = C.LearnableParams()
Par.RegisterAlpha(init_value=0.5)
Par.RegisterBeta(init_value=0)
...
for epoch in range(Epochs):
	...
	pred_y_lo1 = NN_l.forward(x_hi)
	pred_y_lo2 = NN_l.forward(x_hi - Par.beta)
	pred_y_hi = NN_h1.forward(torch.cat([x_hi, pred_y_lo1], dim=1)) * (Par.alpha)
	pred_y_hi += NN_h2.forward(torch.cat([x_hi, pred_y_lo1, pred_y_lo2], dim=1)) * (1 - Par.alpha)
	mse_yh = torch.mean(torch.square(pred_y_hi - y_hi))
	...
	if (epoch + 1) % 500 == 0:
		print(f"Epoch {epoch + 1}, Loss={loss.item():.5f}, Alpha={Par.alpha.item():.5f}, Beta={Par.beta.item():.5f}, MSE_yl={mse_yl.item():.5f}, MSE_yh={mse_yh.item():.5f}")
x_lo.requires_grad = False

prediction_yl1 = NN_l.forward(test_x)
prediction_yl2 = NN_l.forward(test_x - Par.beta)
prediction_yh = NN_h1.forward(torch.cat([test_x, prediction_yl1], dim=1)) * (Par.alpha)
prediction_yh += NN_h2.forward(torch.cat([test_x, prediction_yl1, prediction_yl2], dim=1)) * (1 - Par.alpha)
...
ax.scatter(x_lo, y_lo, marker='o', facecolors='none', edgecolors='blue', s=100, linewidth=2, label='Low-fidelity training data')
ax.scatter(x_hi, y_hi, marker='x', c='red', s=100, linewidth=2, label='High-fidelity training data')
ax.plot(test_x, prediction_yh, label='Multi-fidelity learned CNN (Method II)', color='gold', linewidth=3)
ax.grid()
ax.legend(loc=4)
ax.set(xlim=(0, 1), ylim=(-3, 2), xlabel='$x$', ylabel='$y$')
