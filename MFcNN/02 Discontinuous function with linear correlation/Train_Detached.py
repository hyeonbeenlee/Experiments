...
for epoch in range(Epochs):
    ...
    loss += torch.mean(torch.square(dy_lo - pred_y_lo_grad))
...
optimizer1 = torch.optim.Adam(params=NN_l.parameters(), lr=1e-3)
optimizer2 = torch.optim.Adam(params=NN_h1.parameters(), lr=1e-3)
optimizer3 = torch.optim.Adam(params=NN_h2.parameters(), lr=1e-3)
optimizer4 = torch.optim.Adam(params=Par.parameters(), lr=1e-3)
...
for epoch in range(Epochs):
    ...
    pred_y_lo_grad, = torch.autograd.grad(outputs=pred_y_lo, inputs=x_lo, grad_outputs=torch.ones_like(pred_y_lo),
                                          retain_graph=True, create_graph=True, only_inputs=True)
    mse_dyl = torch.mean(torch.square(dy_lo - pred_y_lo_grad))
    ...
    loss = mse_yl + mse_yh + mse_dyl + l2_lambda * l2_reg
